  ...
  const fetchUserData = async () => {
    ...
    try {
      const apiUrl = process.env.NODE_ENV === 'production'
        ? `https://cristaosweb-e5a94083e783.herokuapp.com/api/listings/users/${userId}`
...
const handleDeleteListing = async (listingId) => {
  ...
  const apiUrl = process.env.NODE_ENV === 'production'
    ? `https://cristaosweb-e5a94083e783.herokuapp.com/api/listings/delete/${listingId}`
...
const handleLike = async (listingId) => {
  ...
  try {
    ...
    setUserListings((prevListings) =>
      prevListings.map((listing) =>
        listing._id === listingId ? { ...listing, likes: data.likes } : listing
      )
    );
  } catch (error) {
...
const handleDeleteComment = async (listingId, commentId, parentCommentId = null) => {
  ...
  const apiUrl = process.env.NODE_ENV === 'production'
    ? parentCommentId
      ? `https://cristaosweb-e5a94083e783.herokuapp.com/api/comments/${commentId}/${parentCommentId}`  // Delete reply
      : `https://cristaosweb-e5a94083e783.herokuapp.com/api/comments/${commentId}`  // Delete parent comment
    : parentCommentId
          ...
          ? {
              ...
              comments: item.comments.filter(
                (comment) =>
                  ...
                  (!parentCommentId || comment.replies.every((reply) => reply._id !== commentId))
              ),
            }
          : item
      )
                ...
                <p style={{textDecoration: "none", textAlign:"justify"}}>
                  {listing.blogContent
                    ? listing.blogContent.split(" ").length > 100
                      ? listing.blogContent.split(" ").slice(0, 100).join(" ") +
                        "..."
                      : listing.blogContent
