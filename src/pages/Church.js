...
const mockChurch = {
  ...
  address: {
    ...
    plusCode: "", // opcional
    mapsUrl:
      "https://www.google.com/maps/search/?api=1&query=-23.65,-46.70", // monta via lat/lng
  },
  // contatos
  contact: {
        ...
        actions={
          <>
            {church.contact.instagram && (
              <a href={church.contact.instagram} target="_blank" rel="noreferrer">
                Instagram
              </a>
            )}
            {church.contact.youtube && (
              <a href={church.contact.youtube} target="_blank" rel="noreferrer">
                YouTube
              </a>
            )}
          </>
        ...
        <div className="ch-grid2">
          ...
          <div>
            ...
            <ul className="ch-list">
              {church.contact.phone && <li>Telefone: {church.contact.phone}</li>}
              {church.contact.whatsapp && (
                <li>WhatsApp: {church.contact.whatsapp}</li>
              )}
              {church.contact.email && <li>E-mail: {church.contact.email}</li>}
              {church.contact.website && (
                <li>
                  Site:{" "}
                  <a href={church.contact.website} target="_blank" rel="noreferrer">
        ...
        <div className="ch-grid2">
          ...
          <div>
            {church.about.statementPdf ? (
              <a className="ch-btn" href={church.about.statementPdf} target="_blank" rel="noreferrer">
                Estatuto / Statement
              </a>
            ) : (
      ...
      <Section title="Ofertas & Doações">
        {church.giving?.page ? (
          ...
          <ul className="ch-list">
            ...
            {church.giving?.bank && (
              <li>
                Banco: {church.giving.bank.bank} · Ag. {church.giving.bank.agency} ·
                Conta {church.giving.bank.account} ({church.giving.bank.type})
              </li>
            )}
          </ul>
        )}
